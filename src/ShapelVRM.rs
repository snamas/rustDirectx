...
impl<'a> DrawObj<'a> for ShapelObject<'a>{
    ...
    fn Update(&mut self, current_sw_heaps:CpD3D12_CPU_DESCRIPTOR_HANDLE,mut transition_barrier_desc: D3D12_RESOURCE_TRANSITION_BARRIER) {
        ...
        unsafe { self.cp_id3d12command_dispacher.command_lists[0].cp_resource_barrier(vec![barrier_desc]) };
        ...
        unsafe { self.cp_id3d12command_dispacher.command_lists[0].0.SetPipelineState( self.pipelineState.0) }
        unsafe { self.cp_id3d12command_dispacher.command_lists[0].0.SetGraphicsRootSignature( self.rootsignature.0) }
        ...
        unsafe { self.cp_id3d12command_dispacher.command_lists[0].0.Close() };
